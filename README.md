...
- Restarting the server erases every room and message. A room is erased when its last participant leaves; the one-hour cleanup also covers abandoned empty rooms.
...
Open `http://localhost:5173` in two browser windows. One person creates a room; others enter the six-character code. Use `Ctrl+C` to stop the server and erase the active rooms.
...
The Render process stores active rooms, names, and messages only in its volatile `Map`. There is no database, disk write, browser storage, cookie, analytics, or permanent history. Restarting or redeploying Render erases all rooms. The app also erases a room when its last participant leaves or after inactivity.
